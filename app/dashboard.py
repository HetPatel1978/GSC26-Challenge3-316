...
st.caption(
    ...
    "features, and imminent-failure (next-30-min) predictions from the trained XGBoost model."
)
...
with col1:
    ...
    axes[1].plot(mdf["trace_day"], mdf["max_predicted_risk"], color=RISK_COLOR, lw=1,
                 label="max predicted failure risk (XGBoost)")
...
st.caption("Same time-based test split across all 4 models (src/eval/dataset.py) -- see docs/03-BASELINE-RESULTS.md.")
